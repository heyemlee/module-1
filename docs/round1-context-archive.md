...
Date range: 2026-06-16 onward
Scope: Historical archive for Module 1 Round 1 showroom intake, layout generation, preliminary cabinet estimate, and customer concept rendering work.
Status: Archive only. The current hot context and next-step guidance live in `ai_ctx.md`.
...
This file is the historical working-memory archive for Module 1 Round 1 MVP.

For current status, guardrails, key files, latest verification, and next-step guidance, start with `ai_ctx.md`.
...
Done (2026-06-19): admin self-service user management implemented. Added ADMIN-only `/admin/users` page plus `GET/POST /api/admin/users` with `requireRole("ADMIN")`, 401/403/409 handling, and `user-admin-repository.ts` list/create support with password hashing through the existing helper. Dashboard shows a Users link for admins. Light real-use polish included login brand from `NEXT_PUBLIC_COMPANY_NAME`, dashboard empty-state CTA, and responsive dashboard/header. Salespeople can now be added from the UI instead of editing the Railway pre-deploy command. Verified with `npm test` (214), `npx tsc --noEmit`, `npm run build`, and end-to-end browser QA against local Docker Postgres: login 200, create 201, duplicate 409, SALES gating 403 + redirect, mobile no-overflow.

Done (2026-06-22): Round 1 Rendering Preferences + cabinet color libraries completed and merged to `main` via PR #6. The default Round 1 workflow is now six steps: `Room`, `Openings`, `Layout`, `Appliances`, `Adjust Positions`, `Rendering Preferences`. `Adjust Positions` focuses on confirming fixed positions and generating cabinet fill; `Generate Rendering` moved into the final Rendering Preferences step. Added `Round1RenderingPreferences` to the showroom form/snapshot with `cabinetStyle` (`EUROPEAN_FRAMELESS` or `AMERICAN_FRAMED`) and `doorColorId`. Added admin-managed, per-company, per-style `CabinetColor` storage with Postgres + in-memory repositories, hardened `cabinet_colors` migration, ADMIN-only `/api/admin/cabinet-colors` APIs, `/admin/cabinet-colors` UI, and dashboard link. The Sales step shows active colors for the selected style only, supports hover example images, requires click-to-confirm before saving a color, clears incompatible color selections when style changes, and enables rendering only once the snapshot is saved and rendering preferences are complete. Rendering prompts now include style-specific cabinet language plus the selected color's `promptDescription`; stored renderings stamp `basedOnRenderingPreferences` alongside `basedOnSnapshotGeneratedAt`. Changing only style/color does not clear cabinet fill or snapshot; it only marks an existing rendering stale. Verification: `npm test` 250 passing, `npx tsc --noEmit`, `npm run build`, local Docker Postgres migration, Admin Cabinet Colors QA, Sales Rendering Preferences QA, and prompt/stored-rendering metadata QA for both European and American styles.

Done (2026-06-22): simplified the Admin Add Color form to upload-based fields. The create form now asks only for Cabinet style, Color name, Swatch image upload, optional Hover example image upload, and optional AI description. Uploads are read client-side into data URLs and stored in the existing `swatch_image_url` / `hover_example_image_url` text columns; no image host or object storage was added. `promptDescription` falls back to the color name when left blank. `colorCode`, `swatchHex`, and `sortOrder` remain nullable columns but are no longer shown in the form; `sortOrder` defaults to 0 and the list sorts by name. `active` is shown only when editing an existing color and defaults true on create. The `cabinetColorInputSchema` accepts either hosted URLs or `data:image/...;base64,` URLs for swatch/hover images, and editing without re-picking a file preserves the stored image. Verified with `npm test` 251 passing, `npx tsc --noEmit`, `npm run build`, and local browser QA: form renders the five fields, image upload posts 201, data URL persists, and the image renders in the Admin board.

Done (2026-06-22): production migration applied against the Railway DB through the public TCP-proxy `DATABASE_URL` (`*.proxy.rlwy.net`). The migration created the `cabinet_colors` table and added `renderings.based_on_*` columns. It is idempotent (`CREATE TABLE` / `ADD COLUMN IF NOT EXISTS`), so the Railway pre-deploy `db:migrate` should be a no-op on redeploy. Note: `.env.local` `DATABASE_URL` points at the Railway public proxy host for local dev against prod data; `scripts/migrate.mjs` does not auto-load `.env.local`, so pass `DATABASE_URL` explicitly when running migrations from local.

Done (2026-06-22): Seeded the 30 in-stock EU board colors into the production `cabinet_colors` library. All seeded rows are `EUROPEAN_FRAMELESS`, active, `sort_order` 1-30, with `hover_example_image_url` left NULL; no `AMERICAN_FRAMED` colors have been seeded yet. Source deck: `有现货的板材颜色 2026.01.06.pdf` (30 pages, one named swatch per page: SYN matte woodgrain, HG high-gloss, SM super-matte, VD, ABC-WG woodgrain, and ABC-OT woven). Added pure data tooling under `scripts/`: `cabinet-colors-eu.data.mjs` (catalog + prompt descriptions), `prepare-cabinet-colors.mjs` (PDF rasterize/crop/resize to generated JSON + contact sheet), and `seed-cabinet-colors.mjs` (idempotent upsert keyed on company/style/name with `DRY_RUN`). Added npm scripts `db:prepare-cabinet-colors` and `db:seed-cabinet-colors`. Verified first against throwaway Docker Postgres, then Railway public-proxy `DATABASE_URL`; production confirmed `total=30 / eu_active=30 / with_jpeg=30 / hover_null=30 / blank_prompt=0`, and the user confirmed colors render on the live Admin board.

Done (2026-06-22): Whole-project navigation/flow/security review hardening and polish merged to `main` through PR #8 and PR #9. PR #8 closed high/UX-completeness issues: Round 1 intake back-to-project link, app-level `not-found`/`error` pages, snapshot-save Retry, `beforeunload` unsaved-edit guard, cabinet-color load-error vs empty-state distinction, and rendering preference persistence before image generation to avoid wrong-color renders. Security hardening deleted unauthenticated legacy `/api/round1/projects/*` and `/api/round1/layout-image` routes, added `requireUser()` to `/api/round1/agent`, and introduced `authErrorResponse()` in `src/server/platform/api-errors.ts` to map auth/body failures consistently across project, Round 1, cabinet-color, and admin routes. Resilience work added OpenAI/client rendering timeouts and safer try/catch/finally handling in login, new project, create user, and cabinet color forms. PR #9 polish added `LogoutButton` across pages, made the project-detail Renderings card less misleading, restored snapshot-derived `fixedPositionsConfirmed`/`cabinetFillGenerated`, made position reset return users to Adjust Positions, prevented logout fetch errors from being treated as logged-out success, switched create-user and cabinet-color forms to `router.refresh()`, aborted stale rendering-preference saves, and removed the orphaned layout-image service/test. Verified on the PR #9 branch with `npm test` 233 passing, `npx tsc --noEmit`, `npm run build`, plus local browser checks of 404/error pages, sign-out buttons, and the Renderings card.

Done (2026-06-22): Rendering color fidelity + renderings history page + admin tidy merged to `main` through PR #10. Sales `Generate Rendering` now sends the selected cabinet color swatch as a material reference image in addition to the deterministic spatial reference: `rasterizeImageSourceToPngBase64` in `rendering-references.ts` converts the swatch to PNG base64 and appends it to `referenceImagesBase64`; `rendering-prompt.ts` adds explicit swatch-matching language whenever `color.swatchImageUrl` exists. Added read-only `/projects/[projectId]/renderings` via `src/features/platform/renderings-view.tsx`, server-rendered from persisted rendering history and cabinet color lookup; the project-detail Renderings card now links there. Admin Cabinet Colors board no longer shows the always-empty "No code" line. Verified with `npm test` 233 passing, `npx tsc --noEmit`, and `npm run build`.

Done (2026-06-22): Admin cabinet-colors performance + batch-save, smaller swatches, and Round 1 "Back to projects" link merged to `main` through PR #11. Root cause: 30 inline base64 swatches made `/admin/cabinet-colors` heavy, and the page hydrated 30 per-row `CabinetColorForm` client components. The Admin board now uses `src/features/platform/cabinet-colors-manager.tsx`: compact 80x80 swatches, inline edits for name/style/active/swatch/hover/AI description, dirty-row highlighting, unsaved-count header, and a single Save All action that calls existing per-color `PUT` routes only for dirty rows. Uploads are downscaled client-side with `resizeImageToDataUrl` (max 512px JPEG q0.82) before storage; swatch/hover images use lazy loading and async decoding in Admin and Sales grids. The Round 1 intake header now includes both "Back to projects" and the project-detail link. Verified with `npm test` 233 passing, `npx tsc --noEmit`, and `npm run build`; browser QA remains useful for `/admin/cabinet-colors` batch edit/save and Round 1 step 6 after deploy.
