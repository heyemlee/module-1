...
Done (2026-06-22): Whole-project navigation/flow/security review + hardening + polish, split across two follow-up PRs to `main`. A multi-agent review (adversarially verified) surfaced ~17 real issues; the **high + UX-completeness** fixes shipped in PR **#8** (`fix/round1-flow-hardening`) and the **low-priority polish** in PR **#9** (`fix/round1-low-priority-polish`, stacked on #8). Related: PR **#7** (`chore/cabinet-color-seed-scripts`) holds the cabinet-color seed tooling; the 30 in-stock EU colors are already live in the prod `cabinet_colors` table (see the seed entry below).
- **Merge/deploy note:** all three PRs target `main` and could NOT be merged from the CLI — `main` has a branch-protection rule and the local `gh` account (`ywutian`) is not a repo admin (`admin:false`). They must be merged in the GitHub UI (by the `heyemlee` owner / a reviewer); merging `main` triggers the Railway redeploy (pre-deploy `db:migrate` is an idempotent no-op).
- **PR #8 — flow/security hardening** (src/ only, +535/−925): Round 1 intake "Back to project" link; app-level `src/app/not-found.tsx` + `error.tsx`; snapshot-save **Retry** control (a failed save no longer disables both Generate Cabinet Fill + Generate Rendering with no recovery); `beforeunload` unsaved-edit guard; cabinet-color load-error vs genuinely-empty distinction (+ retry); rendering now persists the selected finish BEFORE generating (no wrong-color renders). Security: **deleted** the unauthenticated legacy `/api/round1/projects/*` + `/api/round1/layout-image` routes (+ tests + dead client fallback); added `requireUser()` to the live `/api/round1/agent`; shared `authErrorResponse()` (`src/server/platform/api-errors.ts`) maps `requireUser`/`requireRole` failures to 401/403 (and bad bodies to 400) across the project / round1 state·snapshot·renderings / cabinet-colors / admin/ai-status routes (was opaque 500 on session expiry). Resilience: OpenAI image client 90s request timeout + client rendering 120s `AbortController`; try/catch/finally on the login / new-project / create-user / cabinet-color forms.
- **PR #9 — low-priority polish** (stacked on #8): `LogoutButton` now on every page (project-detail, Round 1 header, new-project, admin users + cabinet-colors) — was dashboard-only; project-detail "Renderings" card de-emphasized (dimmed; copy points into Round 1 Intake so it doesn't read as a clickable dead-end); restore now derives `fixedPositionsConfirmed`/`cabinetFillGenerated` from a persisted snapshot (no skewed-state dead step) and resetting positions clamps the user back to Adjust Positions; `logout-button` no longer swallows fetch failures or redirects as logged-out on error; `create-user-form` + `cabinet-color-form` use `router.refresh()` instead of `window.location.reload()` (preserves scroll/sibling-form state; Add forms reset after a successful create; their tests now mock `next/navigation` `useRouter`); `updateRenderingPreferencesForm` aborts the in-flight PUT before a new one (no out-of-order finish saves); removed the orphaned `src/server/round1/layout-image-service.ts` (+ test).
- Verification (PR #9 branch, which includes #8): `npm test` 233 passing, `npx tsc --noEmit` exit 0, `npm run build` exit 0, plus local browser checks of the 404/error pages, the new sign-out buttons, and the de-emphasized Renderings card. Remaining: merge #7/#8/#9 → Railway redeploy → real end-to-end rendering QA on prod (pick 2–3 distinct finishes — deep black / polar white / warm woodgrain — and confirm the chosen color's `promptDescription` reaches the prompt and the prod `OPENAI_API_KEY` works). (The cabinet-color seed work + the 30 live EU colors are recorded in the seed entry further down, added on PR #7's branch.)

Done (2026-06-22): Admin cabinet-colors performance + batch-save, smaller swatches, and a Round-1 "Back to projects" link (branch `fix/cabinet-colors-perf-batch-save`).
- **Root cause of the admin/sales slowness:** each swatch is a ~74–167 KB (avg ~114 KB) `data:image/jpeg;base64` URL stored inline in `cabinet_colors`; 30 EU colors = ~3.4 MB shipped + 30 synchronous `<img>` decodes. The Admin board also hydrated **30 heavy per-row `CabinetColorForm` client components** (each holding its base64 in state), which is what made clicking the page feel very slow.
- **Admin board rewritten to one lightweight batch editor** (`src/features/platform/cabinet-colors-manager.tsx`, replaces the 30 per-row `<details>`+`CabinetColorForm` instances in `cabinet-colors-admin-view.tsx`). Compact **80×80 swatches** in a 3-col grid (was full-width `aspect-square`). Edit name / cabinet style / active / swatch / hover example / AI description inline; per-row dirty highlight + an unsaved-count header with a single **"Save all changes"** button (was one Save per color). Save fires the existing per-color `PUT /api/admin/cabinet-colors/[id]` only for dirty rows via `Promise.all`, then `router.refresh()` — no new API/endpoint. The separate "Add color" form (`CabinetColorForm`, POST) is unchanged.
- **Image payload bounded at the source:** uploads are now downscaled client-side (`resizeImageToDataUrl`, ≤512px JPEG q0.82, exported from `cabinet-color-form.tsx`) before becoming a data URL — used by both the batch manager and the Add-color form. New swatches are a few KB instead of up to the 4 MB cap. (The 30 *already-seeded* swatches stay as-is until re-saved; they'll shrink on next edit, or run a one-off re-encode if needed.)
- **Lazy image decode** (`loading="lazy"` + `decoding="async"`) added to swatch/hover `<img>` in the Admin board and the Sales `rendering-preferences-step.tsx` grid so off-screen swatches don't block paint.
- **Round 1 "Back to projects":** the intake header (`showroom-intake-app.tsx`) previously had only "← Back to project" (→ project detail); it now shows "← Back to projects" (→ `/projects`) with the project link kept alongside.
- Verified: `npm test` 233 passing, `npx tsc --noEmit` exit 0, `npm run build` exit 0. Browser QA NOT run this pass (admin page needs auth + DB) — worth a manual click-through of `/admin/cabinet-colors` (edit several rows → one Save) and Round 1 step 6 after deploy.

Done (2026-06-22): Rendering color fidelity + renderings history page + admin tidy (PR **#10** `feat/round1-rendering-color-and-history`, stacked on #9).
- **Color now reaches the image model as a picture, not just text.** Sales `Generate Rendering` additionally sends the selected door color's **swatch as a MATERIAL reference image**: the client rasterizes the swatch to a PNG (`rasterizeImageSourceToPngBase64` in `rendering-references.ts`) and appends it to `referenceImagesBase64`; `rendering-prompt.ts` adds a "match the cabinet door fronts' color/finish to the provided swatch" sentence whenever `color.swatchImageUrl` exists. Previously the color influenced the render ONLY through the text `promptDescription`. The deterministic floor plan PNG remains the primary spatial reference; the swatch is an extra material reference (gpt-image-2 `images.edit` accepts multiple `image[]`).
- **Renderings history UI.** New read-only page `/projects/[projectId]/renderings` (`src/features/platform/renderings-view.tsx`, server-rendered from `listRenderings` + `listCabinetColors` for door-color-name lookup); the project-detail "Renderings" card is now a real link to it (was a non-functional placeholder). Renderings were ALREADY persisted in the `renderings` table (per-project history) and customers already exist (`customers` table, every `projects` row has a `customer_id`) — this only adds the viewing surface, no new storage.
- **Admin tidy.** Removed the always-"No code" `colorCode` line from the Admin cabinet-colors board (the simplified Add Color form no longer collects `colorCode`; the column stays nullable).
- Verified: `npm test` 233 passing, `npx tsc --noEmit` exit 0, `npm run build` exit 0 (new `/projects/[projectId]/renderings` route compiles).
...
Done (2026-06-22): Seeded the **30 in-stock EU board colors** into the **production** `cabinet_colors` library (Admin → Cabinet Colors). All `EUROPEAN_FRAMELESS` + `active`, `sort_order` 1–30, `hover_example_image_url` left NULL (no US/`AMERICAN_FRAMED` colors yet). Source deck: `有现货的板材颜色 2026.01.06.pdf` (30 pages, one named swatch per page — 11 SYN matte woodgrain, 5 HG high-gloss, 6 SM super-matte, 1 VD, 6 ABC-WG woodgrain, 1 ABC-OT woven).
- New tooling under `scripts/` (pure data tooling; no app/runtime/schema changes):
  - `cabinet-colors-eu.data.mjs` — catalog: `page`→`name`, `promptDescription` (color+finish+grain only; `rendering-prompt.ts` adds the European-style prefix), `swatchHex` fallback.
  - `prepare-cabinet-colors.mjs` — `pdftoppm` rasterize → `sips` centered-square crop (drops the baked-in bottom-corner name label) → 800px JPEG → base64 data URLs → generated `scripts/cabinet-colors-eu.json` (+ a `/tmp/abc-swatches` contact sheet for review). The JSON (~2.2 MB) is **gitignored** and regenerable from the PDF + data module.
  - `seed-cabinet-colors.mjs` — idempotent upsert keyed on `(company_id, cabinet_style, name)`, `DRY_RUN` preview, company resolved by name (prefers "ABC Cabinet", else single-company autodetect, else `SEED_COMPANY_NAME`); mirrors `seed-admin.mjs` (raw `pg`).
  - npm scripts `db:prepare-cabinet-colors` / `db:seed-cabinet-colors`.
- Each swatch is stored as a `data:image/jpeg;base64,…` URL (avg ~74 KB, max ~167 KB — well under the 4 MB admin-form cap), identical to what the admin upload form produces, so no image host/object storage is needed.
- Verified: Phase B first validated against a throwaway Docker Postgres (migrate → seed-admin → seed = 30 rows; re-run = `Created 0, updated 30`, no dups), then run against the Railway public-proxy `DATABASE_URL` — `DRY_RUN` previewed 30 creates, the real run created 30, and `psql` confirmed `total=30 / eu_active=30 / with_jpeg=30 / hover_null=30 / blank_prompt=0`. User confirmed the colors render on the live Admin board.
- Work lives on branch `chore/cabinet-color-seed-scripts`. Follow-ups: fill per-color hover example images via the Admin form; add American-framed colors when in-stock US boards exist; the remaining real-use check is one end-to-end `Generate Rendering` per a few distinct finishes (confirms the selected color's `promptDescription` reaches the prompt and prod `OPENAI_API_KEY` works).
