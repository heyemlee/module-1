...
Status: `todo.md` first-round stages 0–5 and second-round stages 6–9 are complete. Phase 9 verification on 2026-07-06: `npm test` 561 passing / 1 skipped, `npx tsc --noEmit` clean, and `npm run build` succeeds. Only stage 10 remains: the manual three-layout browser QA (galley / L / U locked in Round 1, walked through lock → measure → intent → submit → elevation adjust → drawings → mark reviewed) still needs seeded projects, so it remains a user-side acceptance step.
