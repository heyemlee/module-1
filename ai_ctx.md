...
Status: `todo.md` first-round stages 0–5 and second-round stages 6–7 are complete. Phase 7 verification on 2026-07-06: `npm test` 526 passing / 1 skipped, `npx tsc --noEmit` clean, and `npm run build` succeeds. Stage 8 is next: make autofill consume the collected intent through corner-first, anchored, partitioned solving and height-chain rules. The manual three-layout browser QA (galley / L / U locked in Round 1, walked through lock → measure → submit → adjust → drawings → mark reviewed) still needs seeded projects, so it remains a user-side acceptance step.
