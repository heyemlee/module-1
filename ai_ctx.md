...
- Appliance identity rendering (2026-07-07): `model/segment-role.ts` resolves each segment's semantic role (sink / dishwasher / range / fridge / oven / microwave / hood / fridgeUpper) from the Round 1 fixed point it was reserved for (`sourceFixedPointId` → `symbol`), with the `CABINET_STANDARDS` label prefix (SB/DW/RNG/REF, HD/WR uppers) as fallback for segments that lost their fixed point. `appliance-glyphs.tsx` draws shared SVG line-work fronts (fridge split doors + handles, range knobs + oven window, DW control strip, sink faucet + rim over the door face, hood insert strip, and a divided-light window with a mullion grid — column/row counts derived so panes stay ~square — matching the standard elevation) used by **both** the workspace elevation (`proposal/wall-elevation.tsx`) and the A-sheets (`drawings/drawing-sheet.tsx`). In the 02 Design Proposal workspace, cabinet numbers/codes (including appliance codes such as `RNG30` and `DW24`) are intentionally hidden; they appear only in the 03 Drawings & Review output. Appliance glyphs remain rendering-only and never feed geometry. Tall units (fridge, oven/pantry towers) also carry their own overall height dimension (counter + backsplash + upper, drawn inside the column) since they have no counter/upper split — `data-elevation-layer="tall-height"` in the workspace, `data-drawing-layer="tall-height"` on the A-sheet; both drawn after the cabinet boxes so they're not painted over.
- Proposal interaction redesign (2026-07-08): the 02 stage is a review-first checklist, not a design tool — anyone with submitted measurements gets an autofilled drawing and only confirms/tweaks. The drawing is the **single editing entry point**: clicking a cabinet or chain label opens `SegmentEditorCard` in `proposal/wall-elevation.tsx` (width chips + custom InchField, front presets/hardware/accessories, kind, group nudge — the front/kind controls moved here from the rail). `proposal/decision-rail.tsx` no longer embeds an editor; it is status chip ("READY TO DRAW" / "N TO CONFIRM") + clickable decision checklist (jump via `SELECT_OBJECT`, ceiling-overflow card gets a one-click "step uppers down" fix) + the global height chips + remeasure. **Fillers are remainder space**: their width can't be typed or nudged (`nudgeGroup` now rejects fillers); the filler card only offers placement — `SET_FILLER_PLACEMENT` (`setFillerPlacement` in `model/adjustments.ts`, replacing `MOVE_FILLER_END`) merges all zone fillers to the left end / right end or splits them across both ends, zone-bounded by the shared `zoneBounds` helper. When a cabinet width change is absorbed by a filler, the reducer diffs the model into `state.lastAbsorbed { segmentId, deltaSixteenths, token }` and the elevation pulses that filler with a ±delta badge (`data-elevation-layer="absorb-pulse"`, SMIL animation, rendering-only) so the closed dimension chain is visible instead of feeling like a bug.
- Blocking-decision hard gate (2026-07-08): decision severity now gates advancement. `hasBlockingDecisions(model)` (`model/round2-model.ts`) is true when any `decisionItems[].severity === "blocking"` — a wall run that overflows/doesn't close, or an impossible corner. `RESOLVE_DESIGN_DECISION` returns state unchanged while a blocking decision exists (only advisory confirmations/warnings can be acknowledged to READY), and `SET_TASK` refuses `DRAWINGS` while blocking (`round2-state.ts`). The task nav (`round2-task-navigation.tsx`, `drawingsBlocked` prop) locks the Drawings tab with a "Resolve blocking issues to unlock" tooltip + `BLOCKED` meta, and the rail (`proposal/decision-rail.tsx`) shows a red `BLOCKED` chip, a "wall runs over its measured length" banner, and disables the Resolve button. This closes the earlier hole where Resolve force-set READY regardless of severity. Overflow is only reachable via manual widening past filler slack (autofill clamps appliances to fit); the fix is edit width or request remeasure — appliances stay anchored, there is no free relocation.
- Sink-first anchoring (2026-07-10): in 02 Design Proposal the window-aligned sink cabinet is confirmed **first**. `packReservations` (`model/autofill.ts`) pins the sink to the measured window center and packs every other movable reservation into the intervals left/right of it (dishwasher still docks; ranges/doors yield instead of displacing the sink). Only immutable geometry (corner insets / wall bounds) can knock the sink off center, and that still raises the blocking "Sink placement conflicts with window alignment" decision. The `sink-window.{wall}.alignment` intent question moved to the front of `buildDesignIntentQuestions` so it leads the DESIGN INTENT group and the decision rail; the "Keep Round 1 intent" opt-out remains.
- Upper-corner modeling + top-view corner redraw (2026-07-10): the upper tier now turns corners on its own terms instead of copying the base. Each corner gets a second intent question (`corner.{id}.upper`, kind `corner-upper-strategy`): default `diagonalUpper` = one 24″ `WDC24` diagonal wall cabinet consuming 24″ on both walls (segment on the primary wall + `WDC24 return` gap on the secondary, mirroring the LS pattern); `blindUpper` = straight `WBC27/30` on the primary with the secondary yielding 12″ upper depth + 3″ pull; `openUpper` = no unit, secondary clears 12″. `buildCornerInsets` returns `{ base, upper }` tiers per wall, `deriveUpperTier` fills inside its own insets, and `mapBaseToUpperPiece` treats wall over base-corner bodies (LS return / blind body) as ordinary upper space — the old base-width (36″) "Corner clearance" gaps are gone, recovering ~12–24″ of uppers per corner per wall. Standards: `corner.upperDiagonal` / `corner.upperBlind` in `cabinet-standards.ts`. Top view (`proposal/design-plan.tsx`): diagonal upper corners draw as a dashed chamfered pentagon at 12″ depth via `UpperCornerFootprints` (not a straight rect through the LS glyph), upper fillers now render in the dashed chain, and the secondary-wall "Blind corner" body gap rect is no longer double-drawn over the BB cabinet's own rectangle (its dimension chain entry stays).
- Drawings (`drawings/drawing-sheet.tsx`, `cabinet-schedule.tsx`) are projections of the same model: A1 plan, one elevation sheet per participating wall (so a galley yields 2, not a fixed A2–A4), and an S1 schedule including filler rows plus a FRONT column from `resolveSegmentFront`. Elevation sheets render vertically from the same `heightProfile` and stagger dimension labels through the same `dimension-lanes.ts`, so the drawings match the proposal elevation exactly. `ADOPT_BASIS` carries the full `Round1ReferenceSource` and derives at lock time.
- Fridge surround (per-unit, 2026-07-13): clicking the fridge tall unit (or one of its finished panels) in the 02 elevation opens `FridgeSetupControls` in `SegmentEditorCard` (`proposal/wall-elevation.tsx`) with two rows — **ABOVE FRIDGE** (Open gap / Wall cabinet / Panel) and **SIDE PANELS** (None / Left / Right / Both); all card + drawing labels are English (the panel is the 见光板 concept). Like the corner setup, choices live in design intent keyed per fridge fixed point (`fridge.{fixedPointId}.above` / `.sides`, helpers in `model/design-intent.ts`) and re-run autofill **live** — `SET_DESIGN_INTENT` fires `regenerateProposalFromIntent` for `corner.`/`fridge.` keys whenever the proposal is unlocked (segments exist), **not** only when `measurementStatus === "SUBMITTED"`: any other intent edit drops the status to DRAFT and previously silently disabled the live regen (the original corner controls had the same latent gap). The regen preserves `measurementStatus` (never fakes a submit) and keeps the fridge selected. Autofill (`model/autofill.ts`): the above choice is honored in `mapBaseToUpperPiece`'s tall-unit branch only when the tall unit's symbol is `fridge` (oven/pantry towers keep the plain gap) → `wallCabinet` emits a `W{w}` upper over the fridge, `panel` emits a new `kind: "panel"` upper labeled "Panel", `gap` is the original behavior. Side panels are bundled into the fridge base reservation (block width = fridge + left + right, `fridgeSidePanels` reads `sides`, width `CABINET_STANDARDS.finishedPanel.sideWidthSixteenths` = 3/4″) and split in `fillBaseTier` into `[left panel, fridge, right panel]`, so panels **consume real wall width and shift neighbours** like any reservation and share the fridge's `sourceFixedPointId`. New `WallSegmentKind: "panel"` = a decorative finished panel (no cabinet number, no counter, full height beside a tall unit); rendered in both the workspace elevation (`PanelHatch` diagonal hatch + rotated PANEL label) and the A-sheets. **Capped tall rendering**: both renderers draw base tiers after upper tiers, so a full-height tall box would paint over the unit above it — `upperOccupiedFixedPoints` / `isCappedTall` (`model/round2-model.ts`) detect a wall cabinet/panel above a tall unit and cap the tall box at the upper band's underside (`segmentBox` third arg in `wall-elevation.tsx`, `elevationBox` in `drawing-sheet.tsx`); the tall height chain then reads counter+backsplash (e.g. 54″) instead of the full 90″, in both the workspace (`TallUnitHeights`) and the A-sheet tall-height layer. **Defaults (gap / none) reproduce the prior single-segment fridge byte-for-byte** — all prior round2 tests unchanged; coverage in `autofill.test.ts` (above/sides geometry), `wall-elevation.test.tsx` (card controls, capped-box regression), `round2-state.test.ts` (live regen keeps fridge selected; regen still fires from DRAFT). Verified 2026-07-13: `npm test` 690 passing / 1 skipped, `tsc` clean, reducer-driven SSR browser QA (repro incl. the DRAFT-status path) of all above/side combinations.
- Design basis + phase flow redesign (2026-07-08): the customer-confirmation lock moved out of Round 2 into the renderings page, which is now the "Proposal & confirm" page. Locking a rendering creates a server-persisted, append-only **design basis** (`design_basis` table; `src/server/platform/design-basis-repository.ts`; `POST /api/projects/[id]/design-basis`) = {rendering, its `round1_snapshot_id`, cabinet style, door color} — the exact package the customer saw, never "the latest snapshot". Relocking inserts version+1 and `lockDesignBasis` derives everything server-side from the rendering row. Locking sets project status `ROUND2_MEASURING`, and Round 1 snapshot/rendering saves no longer regress the status after that. Round 2 (`round2/page.tsx`) resolves its reference from the current basis (via `getRound1SnapshotById`), auto-adopts it (`ADOPT_BASIS` with the server version replaced the client-local `LOCK_REFERENCE`/`REPLACE_REFERENCE`/`OPEN_REFERENCE_HANDOFF`; the handoff lock page became the `handoff/basis-gate.tsx` empty state) and reconciles the localStorage draft against it (`reconcileDraftWithBasis` in `round2-draft-storage.ts`): a draft built on the same basis version+snapshot resumes; a mismatched draft is archived under `…draft.{projectId}.v1.archived.basis-v{N}` and the workspace restarts with a dismissible notice — field work is never silently discarded. Locking requires the rendering to carry a style/color stamp; open Confirmation Required items don't block but must be explicitly acknowledged in the lock dialog (count comes from `listRenderings`' joined `confirmationCount`). UI naming: Round 1 → "Concept", renderings → "Proposal & Confirm", Round 2 → "Technical Design" (routes/internal names unchanged); the project detail phases treat a locked basis as the technical-design stage and the next-action button prefers it. Gallery grouping (2026-07-09): the Proposal & Confirm gallery (`renderings-gallery.tsx`) shows **one window per layout**, not one card per rendering — renderings sharing a `round1_snapshot_id` are the same design in different finishes and collapse into a single card the rep pages through by colour (chevrons/dot indicators/touch-swipe on the card, arrow keys in the lightbox; the lightbox navigates within the group). Layout is the decision that anchors technical design; colour is a freely-browsed variant. A layout that holds the basis opens on the **locked finish** (so "which colour did we confirm" is visible) and shows a `BASIS v{n}` marker; swiping to another finish flips the lock row to "Relock basis" so confirming the customer's colour is just locking the finish you paged to (`design_basis.door_color_id` already captures it, so no new lock semantics — `RenderingSummary.round1SnapshotId` is forwarded to the card as `layoutId`). Deferred (M3): diff-based partial invalidation on relock (color-only change keeping measurements), server-side Round 2 state, archived-draft viewer.
