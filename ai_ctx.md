...
- Round 1 appliance detail boundary: cooking appliances capture rough presence for `range`, `cooktop`, `wallOven`, and `microwaveOvenCombo`. `range` and `wallOven` still ask for an approximate wall; `cooktop` and `microwaveOvenCombo` do NOT (their wall is decided by the intelligent auto-layout, then drag-adjustable). Range is treated as an appliance that includes an oven; detailed sizes/types/configuration stay out of Module 1 and belong to a later detailed questionnaire plus designer field measurements.
...
- `npm test`: 140 tests passing (was 116; +24 for the conversational agent's provider/service layers).
...
Done (2026-06-18): Excluded the "Microwave / oven combo" text label from the Top-Down Layout Plan. The physical appliance geometry/symbol is still drawn, but the text label inside is omitted to keep the plan clean per request. Verified: `npm test` (168 passing) and `npx tsc --noEmit`. See `layout-preview.tsx` and `layout-preview.test.tsx`.

Done (2026-06-18): dropped the cooktop / microwave-oven-combo wall questions + added intelligent automatic appliance auto-layout. (1) The Appliances form no longer asks for an "approximate wall" for `cooktop` or `microwaveOvenCombo` (`RoughApplianceFields` gained a `showWall` prop, `false` for those two); `range` and `wallOven` keep their wall question. (2) `placeAppliances` in `plan-geometry.ts` now auto-distributes any appliance with no chosen wall across the layout's allowed walls instead of piling them all on the main run: a new `resolveApplianceWall` returns a fixed wall only for a drag override or an explicit (non-`UNKNOWN`/`NOT_APPLICABLE`/`NO_PREFERENCE`) relation, else `null`; null-wall appliances are then placed onto the wall with the most free linear span (committed appliance load vs wall length), processed cooktop→wallOven→microwave. A cooktop is biased to the main run (`TOP`) since it is a primary cooking surface; tall units (wall oven, microwave) spread to the least-crowded wall. Drag overrides and explicit customer wall choices always win, so the customer can still fine-tune by dragging in `Adjust Positions`. Sink/dishwasher/range/fridge keep their existing deterministic defaults (their default relations are explicit), so all prior geometry behavior/tests are unchanged. Verified: `npx tsc --noEmit`, `npm test` (167; +4: 1 form test in `showroom-intake-app.test.tsx`, 3 geometry tests in `plan-geometry.test.ts` covering distribution, cooktop-on-main-run, and override-wins), `npm run build`, and live browser QA at the `module-1-dev` preview — with Cooktop/Wall oven/Microwave all YES the form shows only "Wall oven approximate wall" (no cooktop/microwave wall fields), and in the default L-shape the wall oven auto-lands on `TOP` while the microwave/oven combo auto-lands on `LEFT` (spread across both legs), no console errors. See `showroom-intake-steps.tsx` (`RoughApplianceFields`) and `plan-geometry.ts` (`resolveApplianceWall`, auto-layout block in `placeAppliances`).
