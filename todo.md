...
Validation: `npm test` 通过(508 passed / 1 skipped)；`npx tsc --noEmit` 通过;
...
- [x] 新建 `src/features/round2/model/cabinet-standards.ts`:纯数据常量 + Zod schema 同形校验
  ...
  - [x] 常规 `upperHeights` 30/36/40;烟机柜 12/15/18/21/24;
        冰箱上柜 12/15/18;`counterHeight` 34½″、`backsplashMin` 18″、
        flat moulding 2–3″(preferred 3″)
  - [x] `fillerMin` ½″ / `fillerPreferred` 3″(替代 `FILLER_MIN_SIXTEENTHS`)
  - [x] `corner` 常量(lazySusan 型号名义 36″ / 实际 envelope 39″;
        盲柜 envelope 39″ + 邻墙 3″ 拉距;两者均高 34½″ / 深 24″)
  - [x] `appliances` 宽度(DW24 / RNG30 / SB 30/33/36 / REF36,
...
- 资料校准来源:`欧柜地柜尺寸参数表(2).pdf`、`欧柜吊柜尺寸参数表(1).pdf`、
  `转角柜板件尺寸(2).xlsx`;毫米板件下料数据留给未来 manufacturing/BOM catalog,
  不混入 autofill 标准。
...
- [ ] **① 转角先行(架构性改动)**:逐墙填充**之前**,先在相邻墙交接处按 intent 做
      转角决议 —— 转角同时消耗两面墙宽度(lazySusan 两侧各 39″ envelope;
      盲柜本墙 39″ 且
...
验收:U 型 + lazySusan intent 提交后,两面墙各让出 39″ envelope 转角段;窗下水槽居中;
