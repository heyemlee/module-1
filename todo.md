...
Validation: `npm test` 通过(506 passed / 1 skipped)；`npx tsc --noEmit` 通过;
...
- [x] 新建 `src/features/round2/model/cabinet-standards.ts`:纯数据常量 + Zod schema 同形校验
  ...
  - [x] `doorRule`(单门 ≤21″ / 对开 ≥24″)、`drawerStacks` 标准抽高组合(如 6/12/12)
  - [x] `upperHeights` 档位(30/36/42)、`counterHeight` 34½″、`backsplashMin` 18″、
        moulding 预留
  - [x] `fillerMin` ½″ / `fillerPreferred` 3″(替代 `FILLER_MIN_SIXTEENTHS`)
  - [x] `corner` 常量(lazySusan 36×36;盲柜本墙最小宽 39″ + 邻墙 3″ 拉距)
  - [x] `appliances` 宽度(DW24 / RNG30 / SB 30/33/36 / REF36,
        替代 `applianceWidth` / `applianceLabel` 内的硬编码)
  - [x] 深度:base 24″ / upper 12″ / tall 24″(供俯视图投影与 A1 平面用)
