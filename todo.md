...
- 见光板:所有暴露的柜列两端(地柜层 + 吊柜层)自动加 3/4″ 端板
  (`panelSpan: "tier"`,与所在层柜体同高);转角、到墙端的门洞、
  自带全高侧板的高柜端不重复加。
- 洗碗机两侧固定加地柜高侧板;冰箱/烤箱塔等高柜两侧默认加全高侧板
  (冰箱可经 `fridge.<id>.sides` intent 按侧关闭,默认从 none 改为 both)。
- 无窗 sink:上方生成与 sink 柜同宽同列的专属吊柜(共享 sourceFixedPointId,
  事实上的 sink+吊柜联动模块),顶与吊柜层齐平,底部抬高至台面净空 24–30″
  (默认 24″ → 默认剖面下柜高 30″,见 `model/sink-upper.ts` +
  `CABINET_STANDARDS.sinkUpper`)。
- 立面高度链:地柜柜身标 34 1/2″(36″ 台面高 − 1 1/2″ 台面板);
  台面画为不标注尺寸的板带(proposal 立面与 A 图纸一致);S1 柜体表
  纳入 Panel 行。
...
- [x] 新建 `src/features/round2/model/autofill.ts`:
  ...
  - [x] 挡板最小值规则(当前标准 ¾″,可常量配置):低于阈值 → 产出 decision item 数据
...
- [x] 新建 `src/features/round2/model/cabinet-standards.ts`:纯数据常量 + Zod schema 同形校验
  - [x] `baseWidths` 宽度档位(9/12/15/18/21/24/27/30/33/36,
        替代 `autofill.ts` 的 `STANDARD_CABINET_WIDTHS_SIXTEENTHS`)
  - [x] `doorRule`(单门 ≤21″ / 对开 ≥24″);资料仅含抽屉板件尺寸,
        不足以定义成品正面抽高组合,已删除原型假设
  - [x] 常规 `upperHeights` 30/36/40/42;烟机柜 12/15/18/21/24;
        冰箱上柜 12/15/18/21/24;地柜体高 34½″ + 台面 1½″ =
        完成面高 36″;`backsplashMin` 18″、
        flat moulding 2–3″(preferred 3″)
  - [x] `fillerMin` ¾″ / `fillerPreferred` 3″ / 常用 3/4/5/6″
        (替代 `FILLER_MIN_SIXTEENTHS`)
  - [x] `corner` 常量(lazySusan 33/36″;盲柜 39/42/45″ + 邻墙 3″ 拉距;
        两者均高 34½″ / 深 24″)
  - [x] `appliances` 客户确认宽度(DW 24;RNG 30/33 默认 30;
        SB 30/33/36/39 默认 36;REF 36),替代 `applianceWidth` /
        `applianceLabel` 内的硬编码;无 unknown fallback
  - [x] 深度:base 24″ / upper 12″ / refrigerator upper 24″ / tall 24″
        (供俯视图投影与 A1 平面用)
- [x] `autofill.ts`、`STEP_CABINET_WIDTH` 档位、`SET_SEGMENT_KIND` 宽度规则全部改读
      标准表 —— 自动填充与 02 微调使用同一档位来源
- [x] 单测:schema 校验;消费方替换后现有 autofill / state 测试不回归
- 权威确认来源:`柜子尺寸数据整理.docx`;`欧柜地柜尺寸参数表(2).pdf`、
  `欧柜吊柜尺寸参数表(1).pdf`、`转角柜板件尺寸(2).xlsx` 保留为制造参考。
  毫米板件下料数据留给未来 manufacturing/BOM catalog,不混入 autofill 标准。
...
- [x] **① 转角先行(架构性改动)**:逐墙填充**之前**,先在相邻墙交接处按 intent 做
      转角决议 —— 转角同时消耗两面墙宽度(lazySusan 按所选 33/36″ 档位;
      盲柜本墙按所选 39/42/45″ 档位且邻墙 3″ filler;deadCorner 双侧 filler),
      把每面墙可填区间的起点内推;
      转角段为一等 segment,进柜体表与决策项
- [x] **② 锚定固定点**:同墙有窗时水槽柜中线对齐窗中线(现有 reservation 机制上
      加对齐规则);灶台对燃气位、冰箱靠墙端保持
- [x] **③ 分区填充 + 功能邻接**:锚点把墙切为区间,区间内铺标准宽;
      DW 紧贴水槽、垃圾拉篮在水槽另一侧、灶台两侧优先抽屉柜、宁少而宽
- [x] **④ 挡板归位**:filler 推到转角侧/墙端,不留两柜中间;宽度落在 ¾″–3″ 时
      先尝试相邻柜换档使其归 0 或 ≥3″,不成才出决策项
- [x] **⑤ 吊柜从地柜派生**(不再独立铺一遍):烟机段对齐灶台段宽度、冰箱上方深柜
      或留空、吊柜尽量与地柜对缝、窗两侧对称
- [x] **⑥ 高度链消费天花实测**:吊柜高 = min(标准档, 天花 − 台面 − 后挡水 − moulding
      预留);flat moulding 规格为推导产物;结果写入 `heightProfile`(阶段 9 消费)
- [x] 单测:闭合性/确定性回归 + 新规则各至少一条断言(转角双墙扣宽、水槽对窗居中、
      filler 不在柜间、96″ 与 108″ 天花得出不同吊柜档)

实现说明(2026-07-06):转角拓扑抽到 `model/corners.ts`(design-intent 与 autofill
共用同一 TL/TR/BR/BL 派生);转角柜宽度档位取"两墙都放得下的最大档"(intent 只选
策略,阶段 9 属性卡可换档);lazySusan 主墙出 corner 柜(新 `cabinetKind:"corner"`,
编号进柜体表)、邻墙出同宽 gap 回返段;盲柜邻墙 = 24″ gap(柜身)+ 3″ 拉距 filler;
deadCorner 双侧 24″ filler 占位。门洞现在同时阻断地柜与吊柜(此前只有吊柜);
灶台对齐 G 水电 marker;DW 按 Round 1 相对方位停靠水槽侧;灶台邻柜标 DB(抽屉柜)、
水槽 intent 侧首柜标 WB(垃圾拉篮),几何不变仅标签,阶段 9 front 字段接管。
吊柜由地柜逐段投影(对缝),烟机 HD 段同宽对齐灶台(chimney intent 则留空)、
冰箱上方 WR 深柜、tall 上方留空,窗/门在投影后开洞,开洞残段 <9″ 降为 filler;
"窗两侧对称"以对缝近似,未做严格镜像。`Round2Model.heightProfile` 新增
(counter/backsplash/upperHeight/moulding),"到顶"无法整闭合时出 reveal 决策项。
挡板 <3″ 先在同区间内把最宽柜降一档吸收,仍 <3″ 才出决策项。

验收:U 型 + lazySusan intent 提交后,两面墙各让出所选 33/36″ 转角段;窗下水槽居中;
...
- [x] `npm test` 全绿(561 passed / 1 skipped);`npx tsc --noEmit` 退出码 0;
      `npm run build` 成功(2026-07-06)
- [~] 浏览器 QA:U 型项目(seed 项目 test7.1,已锁定+提交态)立面微调 → 图纸 已在
      本地 dev server 实测验证(2026-07-06):立面为主编辑面 + 俯视图只读投影(墙总长
      A150/B90/C90);高度链改 42″ → Σ 99/96 触发"超天花"blocking 决策项且立面吊柜
      纵向随之升高(upperTop 58.5→42);Phase 8 转角(TL/TR 死角 filler)/ 灶台 / 水槽 /
...
## 阶段 11 — 02 方案页交互重构(核对清单 + 剩余空间挡板,2026-07-08)

目标:02 阶段"人人可出图"——自动生成为主,人工只核对清单和在图上微调;
图纸渲染(尺寸链/电器柜体样式/编号)已定稿,不动。

- [x] 挡板改为只读剩余空间:`setFillerPlacement`(start/end/split,zone 内挡板
      合并/对半)替代 `MOVE_FILLER_END`;`nudgeGroup` 拒绝挡板;挡板卡无尺寸输入
- [x] 吸收反馈:`STEP_CABINET_WIDTH` 后 reducer 对比模型写入 `lastAbsorbed`,
      立面对吸收差值的挡板脉冲高亮 + ±变化量徽标(`absorb-pulse` 层,仅渲染)
- [x] 单一编辑入口:`SegmentEditorCard`(宽度/门抽/五金/配件/类型/整组挪动)
      收编右栏的重复编辑器;右栏重写为核对清单(状态 chip + 决策卡点击跳转 +
      超天花一键降档 + 全局高度 + 复尺),删除 Selected object / Source geometry /
      Constrained adjustment / Wall balance 区块
- [x] 测试:adjustments(placement×3 + nudge 拒绝)、round2-state(lastAbsorbed
      写入/清空、SET_FILLER_PLACEMENT 闭合)、wall-elevation(脉冲层)、
      decision-rail(清单/无编辑器/全过卡)
- [x] Blocking 决策硬闸(2026-07-08):`hasBlockingDecisions` 区分 severity;
      `RESOLVE_DESIGN_DECISION` 有 blocking 时不放行、`SET_TASK` 拒绝进入 DRAWINGS;
      任务导航锁 Drawings(BLOCKED + tooltip),右栏红色 BLOCKED chip + 横幅 +
      Resolve 禁用。修掉了 Resolve 无视 severity 强刷 READY 的漏洞。溢出只能靠
      改宽/复尺修(电器仍锚定,不做自由移动)。测试:state 硬闸、task-nav BLOCKED
- [x] 浏览器 QA(2026-07-08,test7.1 从零走 lock→量尺→提交):核对清单 11 项、
      点击跳墙;#4 柜 18″→15″ 生成 adj-filler 并脉冲 +3″(总长 150″ 不变);
      挡板卡只读 3″ + 三位置按钮,split→1.5″×2(过窄警告入清单)、合并回左端
      (警告消除);吊柜 42″ 触发超天花 blocking + 一键降档 42→40→36 后消除;
      console 零报错

## 阶段 12 — 设计基准与阶段流重构(概念 → 确认闸门 → 深化,2026-07-08)

目标:客户确认动作落在它实际发生的地方(效果图页),Round 2 只读服务端锁定的
"设计基准",反悔有档可查。UI 命名 Concept / Proposal & Confirm / Technical Design
(路由与内部代码名不变)。

- [x] `design_basis` 表(append-only,project+version 唯一)+
      `design-basis-repository.ts`(锁定时从 rendering 行服务端取
      snapshot/style/color)+ `POST/GET /api/projects/[id]/design-basis`;
      锁定置项目状态 ROUND2_MEASURING,此后 Round 1 保存不再回退状态
- [x] 效果图页升级为确认页:当前基准横幅 + 每卡 Lock/Relock 对话框
      (重锁警告 + 未决确认项须勾选,`listRenderings` join 出 confirmationCount)
- [x] Round 2 删除自身锁定页:`basis-gate.tsx` 空态引导;`ADOPT_BASIS`(服务端
      版本号)替代 LOCK/REPLACE/OPEN_REFERENCE 三个 action;页面按基准的
      `round1_snapshot_id` 取快照(不再取 latest)
- [x] 草稿对账:`reconcileDraftWithBasis`(RESUME / ARCHIVE_AND_ADOPT / ADOPT),
